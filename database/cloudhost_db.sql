...
CREATE TABLE IF NOT EXISTS paket_diskon (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_diskon_paket_hosting FOREIGN KEY (paket_hosting_id) REFERENCES paket_hosting(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS pesanan (
    ...
    paket_diskon_id INT DEFAULT NULL,
    ...
    CONSTRAINT fk_pesanan_diskon FOREIGN KEY (paket_diskon_id) REFERENCES paket_diskon(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO paket_diskon (paket_hosting_id, nama_paket, deskripsi, harga_normal, harga_diskon, tanggal_mulai, tanggal_selesai, status) VALUES
(1, 'Starter Flash Sale', 'Diskon spesial untuk pelanggan baru CloudHost paket Starter.', 49000.00, 35000.00, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 7 DAY), 'aktif'),
(2, 'Business Annual Deal', 'Hemat besar untuk langganan tahunan paket Business.', 1188000.00, 829000.00, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 30 DAY), 'aktif');

INSERT INTO pesanan (user_id, paket_id, paket_diskon_id, domain, metode_pembayaran, project_file, status, total_harga, tanggal_pesanan) VALUES
(2, 2, NULL, 'tokobudi.com', 'Transfer Bank', NULL, 'aktif', 99000.00, '2024-06-10 09:00:00'),
(3, 1, NULL, 'blog-sinta.id', 'E-Wallet', NULL, 'selesai', 49000.00, '2024-05-22 14:30:00'),
(2, 3, NULL, 'cloudbudi.co.id', 'Kartu Kredit', NULL, 'pending', 199000.00, '2024-07-01 10:15:00');
