...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_FILES['project_zip']) || $_FILES['project_zip']['error'] === UPLOAD_ERR_NO_FILE) {
        $errors[] = 'Harap unggah folder project dalam format ZIP.';
    } else {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            if ($file['size'] > $maxFileSize) {
                $errors[] = 'Ukuran file melebihi 10 MB. Harap kompres ulang project Anda.';
            } else {
                $extension = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
                if ($extension !== 'zip') {
                    $errors[] = 'Format file tidak valid. Hanya file ZIP yang diperbolehkan.';
                } else {
                    try {
                        $filename = 'project_' . $_SESSION['customer_id'] . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.zip';
                    } catch (Exception $e) {
                        $errors[] = 'Terjadi kesalahan saat memproses file project.';
                        $filename = '';
                    }
                    ...
                    if ($filename !== '') {
                        ...
                        if (!move_uploaded_file($file['tmp_name'], $targetPath)) {
                            $errors[] = 'Gagal menyimpan file project pada server.';
                        } else {
            ...
            <div class="card shadow border-0">
                <div class="card-body p-5">
                    ...
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <label class="form-label">File Project (ZIP)</label>
                            <input type="file" class="form-control" name="project_zip" accept=".zip" required>
                            <div class="form-text">Unggah folder project Anda dalam format .zip (maksimal 10 MB).</div>
                        </div>
