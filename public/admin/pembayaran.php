...
$transactions = mysqli_query($conn, "SELECT t.*, p.domain, p.status AS pesanan_status, p.total_harga, pk.nama_paket, pk.harga AS harga_reguler, pd.nama_paket AS nama_diskon, u.nama, u.email FROM transaksi t JOIN pesanan p ON t.pesanan_id = p.id JOIN users u ON p.user_id = u.id JOIN paket_hosting pk ON p.paket_id = pk.id LEFT JOIN paket_diskon pd ON p.paket_diskon_id = pd.id ORDER BY t.transaction_time DESC");
?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    <span><?php echo htmlspecialchars(!empty($trx['nama_diskon']) ? $trx['nama_diskon'] : $trx['nama_paket']); ?></span>
                                                ...
                                                <span class="small text-muted d-block mt-1"><?php echo htmlspecialchars($trx['domain']); ?></span>
                                            </td>
